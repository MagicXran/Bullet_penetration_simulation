...
// Use regex to find and replace the cleanupOrphanedProcesses method
const methodRegex = /private async cleanupOrphanedProcesses\(\): Promise<void> \{[\s\S]*?^\s{2}\}/m;

const newMethod = `private async cleanupOrphanedProcesses(): Promise<void> {
    try {
      const pids: number[] = [];
...
// Find the method with a simpler approach - find start and end
const startMarker = 'private async cleanupOrphanedProcesses(): Promise<void> {';
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log('Could not find method boundaries. startIdx:', startIdx, 'endIdx:', endIdx);
}
