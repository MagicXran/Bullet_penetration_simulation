const fs = require('fs');
const path = 'C:/Users/ZDxia/.claude/plugins/marketplaces/thedotmack/src/services/process/ProcessManager.ts';

const newContent = `import { existsSync, readFileSync, writeFileSync, unlinkSync, mkdirSync } from 'fs';
...
export class ProcessManager {
  static async start(port: number): Promise<{ success: boolean; pid?: number; error?: string }> {
    ...
    // 先检查端口是否被占用（Windows 兼容）
    const portStatus = await this.isPortInUse(port);
    if (portStatus.inUse) {
      // 尝试杀死占用端口的进程
      ...
      if (!killed) {
        return {
          success: false,
          error: \`Port \${port} is in use by PID \${portStatus.pid} and could not be freed\`
        };
      }
    }
  ...
  /**
   * 检查端口是否被占用（Windows/Unix 兼容）
   */
  private static async isPortInUse(port: number): Promise<{ inUse: boolean; pid?: number }> {
    try {
      if (IS_WINDOWS) {
  ...
  /**
   * 通过端口号杀死占用进程（Windows/Unix 兼容）
   */
  private static async killProcessByPort(port: number): Promise<boolean> {
    const portStatus = await this.isPortInUse(port);
    if (!portStatus.inUse || !portStatus.pid) return true;

    try {
      if (IS_WINDOWS) {
        // Windows: 使用 taskkill 强制终止
        spawnSync('taskkill', ['/F', '/PID', String(portStatus.pid)], {
          stdio: 'ignore',
          windowsHide: true
        });
      } else {
        // Unix: 使用 SIGKILL
        process.kill(portStatus.pid, 'SIGKILL');
      }

      // 等待端口释放
      for (let i = 0; i < 10; i++) {
        await new Promise(r => setTimeout(r, 500));
        const status = await this.isPortInUse(port);
        if (!status.inUse) return true;
      }
    } catch {
      // 忽略错误
    }
  ...
  static async stop(timeout: number = PROCESS_STOP_TIMEOUT_MS): Promise<boolean> {
    ...
    try {
      if (IS_WINDOWS) {
        ...
        spawnSync('taskkill', ['/F', '/PID', String(info.pid)], {
          stdio: 'ignore',
          windowsHide: true
        });
      } else {
      ...
      try {
        if (IS_WINDOWS) {
          // Windows: 如果 taskkill 失败，尝试用 PowerShell
          spawnSync('powershell', ['-Command', \`Stop-Process -Id \${info.pid} -Force -ErrorAction SilentlyContinue\`], {
            stdio: 'ignore',
            windowsHide: true
          });
        } else {
...
console.log('ProcessManager.ts updated successfully!');
